//this simulation measures fk cluster crossings in a hexagon with independently wired bottom/upper-right/upper-left sides and free top/bottom-left/bottom-right sides
#include "stdlib.h"
#include "stdio.h"
#include "math.h"

#define INTEGER 17 //between 1 and 2*RMAX+1, inclusive
#define SIDELENGTHSUM 1024
#define RMAX 16
#define SEED 170
#define RUNFILE "RunFile.txt"
#define	OUTFILE "HexFkQ3XingIndep.txt"
#define Q 3
#define PRINTFREQ 32767
#define RUNSMAX 4194304
#define PROB 0.467911
		...
		for (x = 1; x < length-1; ++x)
			for (y = 1; y < height-1; ++y)
				if ((m = lat[x][y]) < Q)
				{
                    ...
                    lat[x][y] = l;
                    find(x, y, l, m);
                    if (BottomFlag == 1 && UpperRightFlag == 1 && UpperLeftFlag == 1) ++AllXing;
                    else if (BottomFlag == 1 && UpperRightFlag == 1 && UpperLeftFlag == 0) ++BottomUpperRightXing;
                    else if (BottomFlag == 1 && UpperRightFlag == 0 && UpperLeftFlag == 1) ++BottomUpperLeftXing;
                    else if (BottomFlag == 0 && UpperRightFlag == 1 && UpperLeftFlag == 1) ++UpperLeftUpperRightXing;
				}
